...
int main(void) {
  ...
  printf("INITIAL: ");
  if (print_scheduler() == -1) {
    syslog(LOG_ERR, "sched_getscheduler() failure: %s", strerror(errno));
    return -1;
  }

  /**
   * Creating thread attributes, setting priority and CPU core pinning
   */
  ...
  /**
   * Initialise thread indices and execute the sum function on different threads
   */
