...
// sequence a set of periodic services in Linux user space without specialized
// hardware like an auxiliary programmable interval timere and/or real-time
// clock.  For problems similar to and including the final project in real-time
// systems.
//
...
// 5) Linux kernel mostly runs on core 0, but does load balance non-RT workload
// over all cores 6) check for irqbalance
// [https://linux.die.net/man/1/irqbalance] which also distribute IRQ handlers
//
// What we really want in addition to SCHED_FIFO with CPU core affinity is:
//
// 1) A reliable periodic source of interrupts (emulated by delay in a loop
// here) 2) An accurate (minimal drift) and precise timestamp
//    * e.g. accurate to 1 millisecond or less, ideally 1 microsecond, but not
//    realistic on an RTOS even
//    * overall, what we want is predictable response with some accuracy
//    (minimal drift) and precision
//
// Linux user space presents a challenge because:
//
// 1) Accurate timestamps are either not available or the ASM instructions to
// read system clocks can't
//    be issued in user space for security reasons (x86 and x64 TSC, ARM STC).
// 2) User space time with clock_gettime is recommended, but still requires the
// overhead of a system call 3) Linux user space is inherently driven by the
// jiffy and tick as shown by:
//    * "getconf CLK_TCK" - normall 10 msec tick at 100 Hz
//    * cat /proc/timer_list
// 4) Linux kernel space certainly has more accurate timers that are high
// resolution, but we would have to
//    write our entire solution as a kernel module and/or use custom kernel
//    modules for timekeeping and selected services.
// 5) Linux kernel patches for best real-time performance include RT PREEMPT
// (http://www.frank-durr.de/?p=203) 6) MUTEX semaphores can cause unbounded
// priority inversion with SCHED_FIFO, so they should be avoided or
...
//    * POSIX sempaphores do have inversion safe features, but they do not work
//    on un-patched Linux distros
//
// However, for our class goals for soft real-time synchronization with a 1 Hz
// and a 10 Hz external clock (and physical process), the user space approach
// should provide sufficient accuracy required and precision which is limited by
// our camera frame rate to 30 Hz anyway (33.33 msec).
//
// Sequencer - 100 Hz
...
//
// 3) The Jetson NANO requiress a sysctl setting to allow for SCHED_FIFO to be
// used:
//
//    sysctl -w kernel.sched_rt_runtime_us=-1
//
//    See -
//    https://forums.developer.nvidia.com/t/pthread-setschedparam-sched-fifo-fails/64394/3
//
...
//
// 6) For determinism, you should use CPU affinity for AMP scheduling.  Note
// that without specific affinity,
//    threads will be SMP by default, annd will be migrated to the least busy
//    core, so be careful.
...
// Of the available user space clocks, CLOCK_MONONTONIC_RAW is typically most
// precise and not subject to updates from external timer adjustments
//
// However, some POSIX functions like clock_nanosleep can only use adjusted
...
//
// "sudo apt-get install adjtimex" for an interesting utility to adjust your
// system clock
//
//

// only works for x64 with proper privileges - has worked in past, but new
// security measures may prevent use
...
void main(void) {
  ...
  rc = pthread_create(&threads[1], &rt_sched_attr[1], Service_2,
                      (void *)&(threadParams[1]));
  if (rc < 0)
  ...
  rc = pthread_create(&threads[2], &rt_sched_attr[2], Service_3,
                      (void *)&(threadParams[2]));
  if (rc < 0)
  ...
  rc = pthread_create(&threads[3], &rt_sched_attr[3], Service_4,
                      (void *)&(threadParams[3]));
  if (rc < 0)
  ...
  rc = pthread_create(&threads[4], &rt_sched_attr[4], Service_5,
                      (void *)&(threadParams[4]));
  if (rc < 0)
  ...
  rc = pthread_create(&threads[5], &rt_sched_attr[5], Service_6,
                      (void *)&(threadParams[5]));
  if (rc < 0)
  ...
  rc = pthread_create(&threads[6], &rt_sched_attr[6], Service_7,
                      (void *)&(threadParams[6]));
  if (rc < 0)
  ...
  // correct POSIX SCHED_FIFO priorities compared to non-RT priority of this
  // main program.
  //
  // sleep(1);
...
void Sequencer(int id) {
  ...
  // clock_gettime(MY_CLOCK_TYPE, &current_time_val);
  // current_realtime=realtime(&current_time_val); printf("Sequencer on core %d
  // for cycle %llu @ sec=%6.9lf\n", sched_getcpu(), seqCnt,
  // current_realtime-start_realtime); syslog(LOG_CRIT, "Sequencer on core %d
  // for cycle %llu @ sec=%6.9lf\n", sched_getcpu(), seqCnt,
...
void *Service_1(void *threadp) {
  ...
  {
    // wait for service request from the sequencer, a signal handler or ISR in
    // kernel
    sem_wait(&semS1);
